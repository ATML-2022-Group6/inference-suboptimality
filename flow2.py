...
def build_aux_flow(hps: HyperParams):
  ...
  def sample(rng, z0, x, params):
    ...
    zx = jnp.concatenate((z0, x), axis=1)
    ...
    zx = jnp.concatenate((zT, x), axis=1)
  ...
  def _norm_flow(z, v, x, params):
    ...
    μ1, logit_1 = flow_net(net1_params, jnp.concatenate((z, x), axis=1))
    σ1 = nn.sigmoid(logit_1)
    v = v*σ1 + μ1 

    # Equation (10) in paper. No difference doing z_1 or z_2 first in the coupling.
    μ2, logit_2 = flow_net(net2_params, jnp.concatenate((v, x), axis=1))
